...
# S_r.R
# -----------------------------------------------------------------------------
# Computes the quantity S_r(t) used in the ridged local linear estimator
# of the mean function mu(t), following Lin & Wang (2022), Section 2.1.
#
# Definition:
#
#   S_r(t) = sum_{i=1}^n w_i sum_{j=1}^{m_i}
#              K_{h_mu}(T_ij - t) * ((T_ij - t)/h_mu)^r
#
# for r = 0, 1, 2.
#
...
compute_Sr <- function(t_vec, T_list, h_mu, r, kernel, scheme) {
  ...
  # flatten subject-specific times and attach subject weights
  ...
  # --- compute S_r(t) for each t in t_vec ------------------------------------
  S_r <- rowSums(K_mat * (U^r) * matrix(W_all,
                                        nrow = length(t_vec),
                                        ncol = length(T_all),
                                        byrow = TRUE))
  
  # backward compatible: return scalar if input was scalar
  if (length(t_vec) == 1) return(S_r[1])
  return(S_r)
}
